...
switch ($mode)
{
	...
	case 'privacy':
		...
		$template->assign_vars(array(
			...
			'U_BACK'				=> append_sid("register.$phpEx", 'mode=login'),
	...
	case 'delete_cookies':
		...
		if (confirm_box(true))
		{
			...
			$message = $user->lang['COOKIES_DELETED'] . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("index.$phpEx") . '">', '</a>');
			trigger_error($message);
		}
		else
		{
	...
	case 'switch_perm':
		...
		$message = sprintf($user->lang['PERMISSIONS_TRANSFERRED'], $user_row['username']) . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("index.$phpEx") . '">', '</a>');
		trigger_error($message);
	...
	case 'restore_perm':
		...
		$message = $user->lang['PERMISSIONS_RESTORED'] . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("index.$phpEx") . '">', '</a>');
		trigger_error($message);
...
if (!$user->data['is_registered'])
{
	...
	{
		$redirect_url = append_sid("register.$phpEx?i=pm&p=" . request_var('p', 0));
...
$module->assign_tpl_vars(append_sid("register.$phpEx"));
